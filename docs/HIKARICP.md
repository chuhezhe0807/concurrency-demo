...
### 一个大前提：公式假设连接拿到就一直在烧 CPU

上面两条公式都隐含「连接借出后基本都在让 DB 干 CPU 活」。一旦连接持有期间有大量**等待**——慢查询、跨服务调用、或本项目注入的 `sim-db-latency` 那个 sleep——连接大半时间在空等、并不占用 DB CPU，这时就需要比公式更多的连接去填满这些等待空档。这正是 US-011 里 30 连接打爆 10 连接（21.9→69.7 req/s）的原因：不是公式失效，而是 5ms sleep 把负载从「CPU 密集」扭曲成了「等待密集」。判断方法很简单——连接的「持有时间 ÷ 真正用 CPU 的时间」越大，越要往公式上限之外加连接。

### 这台机器（8 核）实际设多少

本机 8 核，且 MySQL 容器**不限 CPU、与应用共享同一 8 核**。分两种口径：

| 口径 | 建议 max-pool-size | 理由 |
|------|--------------------|------|
| 演示（`sim-db-latency=5`，放大对比） | 30（即 `hikari-tuning=true`） | 连接大半在 sleep，多开能填满空档，纯为演示曲线，**别当生产值** |
| 真实（`sim-db-latency=0`） | **10~16，从 12 起步** | 公式给 `8×2+1≈17`，但 DB 与 app 抢同一 8 核，开太多两边互抢 CPU 更糟，往下取 |

调法（比任何公式都准）：设好起点后压测，盯报告里的 `waiting`（等连接的线程数）和 TP99——`waiting` 长期 >0 说明不够、可小步加；加上去 TP99 不降甚至反升，就是到头了、退回上一档。
